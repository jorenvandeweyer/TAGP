...
get_data() ->
  ResourceInstances = digitaltwin_server:get_ResourceInstances(),
  case ResourceInstances of
    ...
    [ResInst| _] ->
      ...
      Result = lists:reverse(get_data(maps:keys(Connectors), ResourceInstances, [])),
      [#{type=>element(1,N), pid=>convert:pid_to_bin(element(2,N))} || N <- Result]
...
get_data([Connector | Remainder], ResourceInstances, Result) ->
  ...
  case lists:member(Res, Result) of
    ...
    false -> get_data(Remainder, ResourceInstances, [{element(1,Res), element(2,Res)} | Result])
  end.
...
add_resource(pump) ->
  {_, FirstResInst} = digitaltwin_server:get_FirstResourceInst(),
  {_, LastResInst} = digitaltwin_server:get_LastResourceInst(),
  {ok, Pump_Pid} = make_pump(fun  (on) -> switched_on;
                                  (off) -> switched_off
                             end),
  if
...
get_resource_data(ResInst_Pid) ->
  PumpTyp_Pid = digitaltwin_server:get_ResourceType(pumpTyp),
  {ok, ResType} = resource_instance:get_type(ResInst_Pid),
  {ok, Func} = resource_instance:get_flow_influence(ResInst_Pid),
  if
    PumpTyp_Pid =:= ResType ->
      #{flow_influence=>Func(100), onOrOff=>element(2,pumpInst:is_on(ResInst_Pid))};
    true-> #{flow_influence=>Func(100)}
  end.
